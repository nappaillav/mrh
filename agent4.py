...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        goal_rep_fn = partial(goal_rep, state_dim=self.zs_dim, 
                            encoder_module=None, goal_dim=self.goal_dim)
        # choice of world model
        if self.enc_version == 'V2':
            num_bins = self.num_bins if self.two_hot_loss else 1
            self.encoder = WorldModel2H(zs_dim=self.zs_dim, action_dim=action_dim, 
        ...
        elif self.enc_version == 'V2R':
            num_bins = self.num_bins if self.two_hot_loss else 1
            self.encoder = WorldModelV2R2H(zs_dim=self.zs_dim, action_dim=action_dim, 
                                        state_encoder=Encoder_module, 
                                        goal_rep_fn=goal_rep_fn, sigmoid=self.sigmoid_fn, 
                                        ln=self.lengthnorm_fn, num_bins=num_bins).to(self.device)
        elif self.enc_version == 'MrHi':
            print("Using MrHi")
            num_bins = self.num_bins if self.two_hot_loss else 1
            self.encoder = WorldModel_MrHi(zs_dim=self.zs_dim, action_dim=action_dim, 
                                        ...
                                        goal_rep_fn=goal_rep_fn, sigmoid=self.sigmoid_fn, 
                                        ln=self.lengthnorm_fn, num_bins=num_bins).to(self.device)
        elif self.enc_version == 'sMrHi':
            # No high-level inverse model
            print("Using Simple MrHi")
            num_bins = self.num_bins if self.two_hot_loss else 1
            self.encoder = WorldModel_sMrHi(zs_dim=self.zs_dim, action_dim=action_dim, 
                                        state_encoder=Encoder_module, 
                                        goal_rep_fn=goal_rep_fn, sigmoid=self.sigmoid_fn, 
                                        ln=self.lengthnorm_fn, num_bins=num_bins).to(self.device)
            ...
            self.encoder = WorldModel_MrHiAcHd(zs_dim=self.zs_dim, action_dim=action_dim, 
                                        ...
                                        ln=self.lengthnorm_fn, num_bins=num_bins, 
                                        act_chunk=self.act_chunk).to(self.device)
        elif self.enc_version == 'MrHiAcSg':
            print("Using MrHiAC-Subgoal")
            num_bins = self.num_bins if self.two_hot_loss else 1
            self.encoder = WorldModel_MrHiAcSg(zs_dim=self.zs_dim, action_dim=action_dim, 
                                        state_encoder=Encoder_module, 
                                        goal_rep_fn=goal_rep_fn, sigmoid=self.sigmoid_fn, 
                                        ln=self.lengthnorm_fn, num_bins=num_bins, 
                                        act_chunk=self.act_chunk).to(self.device)
        ...
        # Value
        self.value = Value(state_dim=self.zs_dim+self.goal_dim, gc_encoder=value_encoder_def).to(self.device)
        ...
        self.low_actor = GCActor(state_dim=self.zs_dim+self.goal_dim, action_dim=action_dim, gc_encoder=low_actor_encoder_def, 
                                 state_dependent_std=False, const_std=True).to(self.device)
    ...
    def train_encoder_MrHiAcSg(self, batch):
        """
        Training the Encoder for MrHi version
        """
        with torch.no_grad():
            ...
            sg_target = self.encoder_target.zs(
                batch["sub_goal"].reshape(-1, *self.state_shape)
            ).reshape(batch["state"].shape[0], -1, self.zs_dim)
            zs_target = self.encoder_target.zs(batch["state"][:, 0])
            phi_target = self.encoder_target.phi(zs_target, sg_target[:, 0]) # Phi_target(zs_0, subgoal_0)
        ...
        prev_not_done = 1 # In subtrajectories with termination, mask out losses after termination.
        encoder_loss = 0 # Loss is accumluated over enc_horizon.
        dyn_losses, low_inv_losses, high_inv_losses, return_losses, phi_losses = [], [], [], [], []
        ...
        for i in range(self.enc_horizon):
            ...
            phi_loss = masked_mse(pred_phi, phi_target, prev_not_done)

            if i < (self.enc_horizon-1):
                phi_target = self.encoder_target.phi(encoder_target[:,i], sg_target[:, i+1]).detach()

            encoder_loss = encoder_loss + self.dyn_weight * dyn_loss + 0.1 * return_loss + 0.1 * inv_loss_l + 0.1 * inv_loss_h + 0.1 * phi_loss
            prev_not_done = batch['not_done'][:,i].reshape(-1,1) * prev_not_done
        ...
        metrics = {
                    ...
                    'encoder_HInvLoss': np.mean(high_inv_losses),
                    'encoder_VaLoss': np.mean(return_losses),
                    'encoder_PhiLoss': np.mean(phi_losses),
                }
        return metrics
        ...
        if (self.training_steps-1) % self.target_update_freq == 0:
            ...
            for _ in range(self.target_update_freq):
                ...
                if self.enc_version == 'MrHi':
                    ...
                    self.encoder_metric = self.train_encoder_MrHiAcSg(batch)
                else:
