...
class Hyperparameters:
    ...
    idyn_weight:float = 0.1
    hinv_weight:float = 0.2
    hdyn_weight: float = 0.2 # in Lambda
    ...
    # Model Architecture
    n_layer:int = 2
    stop_goal_grad:bool = False
    stop_hl_grads:bool = False
...
class Agent:
    ...
    def train_encoder(self, batch):
        ...
        if self.stop_goal_grad:
            goal_zs = self.encoder.zs(batch['goal']).detach()
        else:
        ...
        for i in range(self.enc_horizon):
            ...
            encoder_loss = encoder_loss + self.dyn_weight * dyn_loss + self.reward_weight * reward_loss + self.idyn_weight * inv_loss
    ...
    def train_encoder_MrHi(self, batch):
        ...
        for i in range(self.enc_horizon):
            ...
            if not self.two_hot_loss:
                ...
                # (F.softmax(pred_r, dim=-1) * self.two_hot.bins).sum(1, keepdim=True) - batch['reward'][:, i].unsqueeze(-1)
                return_loss = (self.two_hot.cross_entropy_loss(pred_r, batch['reward'][:,i].reshape(-1, 1)) * prev_not_done).mean()
            encoder_loss = encoder_loss + self.dyn_weight * dyn_loss + self.reward_weight * return_loss + self.idyn_weight * inv_loss_l + self.hdyn_weight * inv_loss_h
            prev_not_done = batch['not_done'][:,i].reshape(-1,1) * prev_not_done
            dyn_losses.append(dyn_loss.item())
    ...
    def train_encoder_MrHiAc(self, batch):
        ...
        for i in range(self.enc_horizon):
            pred_zs, pred_al, pred_ah, pred_r = self.encoder.model_all(pred_zs, encoder_target[:,i], 
                                                             goal_zs, batch['action'][:,i], self.stop_hl_grads)
            dyn_loss = masked_mse(pred_zs, encoder_target[:,i], prev_not_done)
            inv_loss_l = masked_mse(pred_al, batch['action'][:,i], prev_not_done)
            ...
            if not self.two_hot_loss:
                ...
                return_loss = (self.two_hot.cross_entropy_loss(pred_r, batch['reward'][:,i].reshape(-1, 1)) * prev_not_done).mean()
            encoder_loss = encoder_loss + self.dyn_weight * dyn_loss + self.reward_weight * return_loss + self.idyn_weight * inv_loss_l + self.hinv_weight * inv_loss_h
    ...
    def train_encoder_MrHiAcHd(self, batch):
        ...
        for i in range(self.enc_horizon):
            pred_zs, pred_al, pred_ah, pred_r, pred_zs_h = self.encoder.model_all(pred_zs, encoder_target[:,i], 
                                                             goal_zs, batch['action'][:,i], self.stop_hl_grads)
            dyn_loss = masked_mse(pred_zs, encoder_target[:,i], prev_not_done)
            inv_loss_l = masked_mse(pred_al, batch['action'][:,i], prev_not_done)
            inv_loss_h = masked_mse(pred_ah, batch['action'][:,i:i+self.act_chunk].reshape(-1, self.action_dim * self.act_chunk), prev_not_done)
            ...
            high_dyn_loss = masked_mse(pred_zs_h, encoder_target[:,i], prev_not_done).mean()
            encoder_loss = encoder_loss + self.dyn_weight * dyn_loss + self.reward_weight * return_loss + self.idyn_weight * inv_loss_l + self.hinv_weight *inv_loss_h + self.hdyn_weight * high_dyn_loss
